...
> An API contract is a promise: “If you send X in this format to this endpoint, you will receive Y or Z error.”
...
> FastAPI’s strength lies in declarative, explicit binding of HTTP request data to function parameters.
...
Results return → ORM maps rows → Python objects.
...
Closing the window doesn’t destroy the building.

**What actually happens?**
Session fetched data → stored in Python
...
If `user_id` is not an integer → FastAPI returns `422 Unprocessable Entity`.
...
- Optional[str] = None means “this field is not required, but if provided, it must be a string.”

- Optional[str] means the field may be missing or may be None; it does not mean “use None instead of the value.”
...
That’s it. JWT is like an ID card that contains your name, role, and an expiry date.
The security guard does not remember who entered or track visits; it simply checks the ID card’s authenticity and validity each time and allows entry.
...
Silent data loss
→ “Data that should exist never gets saved, and nobody notices.”

Unprocessed events
→ “Something happened, but the follow-up work never ran.”

Operational backlog
→ “Failed work keeps piling up until humans must clean it.”

Degraded downstream services
→ “Other systems work with missing or delayed data.”
